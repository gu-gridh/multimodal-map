...
<template>
  <ol-interaction-select @select="onClick" :condition="selectCondition" :features="selectedFeaturesCollection">
    <ol-style>
      <ol-style-icon :src="markerIconRed" :scale="1.9" :displacement="[-9, 47]" :anchor="[0.0, 0.0]"></ol-style-icon>
    </ol-style>
  ...
  <ol-overlay v-if="hoveredFeature" :position="hoverCoordinates">
    <div class="ol-popup" v-html="'Tomb ' + getFeatureDisplayName(hoveredFeature)"></div>
  </ol-overlay>

  <ol-overlay v-if="selectedFeature" :position="selectedCoordinates">
    <div class="ol-popup" v-html="'Tomb ' + getFeatureDisplayName(selectedFeature)"></div>
  </ol-overlay>
</template>
