<template>
  <div class="control-organisation justify-space">
    ...
    <div class="tag-section">
      ...
      <div class="broad-controls">
        <button class="p-0.5 px-2 clickable category-button" :class="{ 'active': selectedBuildingTypeIndex === 0 }"
          @click="selectCategory('building', 0)" :disabled="builderLayerVisible">
          {{ $t('allbuildings') }}
        </button>
        <button v-for="(type, index) in buildingTypes" :key="index" class="p-0.5 px-2 clickable category-button"
          :class="{ 'active': selectedBuildingTypeIndex === index }" @click="selectCategory('building', index)"
  ...
  <RangeSlider ref="rangeSliderRef" v-model="selectedRange" :min="1300" :max="1899" :step="1" class="my-2"
    :isSliderVisible="true" :disabled="builderLayerVisible" />

  <div class="toggle-buttons" style="margin-top: 20px">
    <button style="float:left; border-radius:4px 0px 0px 0px" :class="{ active: searchType === 'places' }"
      @click="setSearchType('places')">{{ $t('places') }}</button>
    <button style="border-radius:0px 4px 0px 0px" :class="{ active: searchType === 'builders' }"
      @click="setSearchType('builders')">{{ $t('builders') }}</button>
  </div>
  <div class="search-section">
    <input ref="searchInput" type="text" v-model="searchQuery" @input="handleSearch" @focus="handleSearchBoxFocus"
      :placeholder="searchType === 'places' ? $t('searchPlaces') : $t('searchBuilders')" class="search-box" autofocus />
