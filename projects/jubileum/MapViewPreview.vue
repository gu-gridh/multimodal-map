...
watchEffect(async () => {
  if (selectedFeature.value) {
    ...
    images.value = await diana.listAll("image", { place });
...
<template>
  ...
  <div v-if="selectedFeature" class="mapview-preview" @click="deselectPlace">
    ...
    <!-- if there are images -->
    <router-link v-for="image in images" :key="image.uuid" :to="`/detail/image/${image.id}`" class="clickable">
      <div class="image-card">
        <div class="image-card-metadata">
          <div class="image-container">
            <img :src="`${image.iiif_file}/full/500,/0/default.jpg`" class="image" />
          </div>
